...
class PostManifest(command.Run):
    """
    Gather list of installed RPMs and compare against a previously taken :class:`PreManifest`
    """
    ...
    def run(self, runner):
        ...
        for task in runner._tasks:
            if isinstance(task, PreManifest):
                ...
                break
        if not pre_manifest:
            return TaskResult(self, success=False, output="You must use PreManifest before PostManifest")

        # ok, so now we have something to compare against so we get new state...
