...
#there's a python egg for this, but it was easier for me to just throw it in the src dir
...
class Nagios:
    ...
    def __init__(self, nagios_instance='http://monitor.mgmt.stage.redhat.com/nagios/cgi-bin/cmd.cgi'):
        """
        You need a Kerberos ticket before running this. I'm not going to pass passwords over HTTP.
        """
    ...
    def _call_nagios_alerts(self, host, command):
        """
        build up the POST call for an enable or disable. Example of what nagios expects to get:
        cmd_typ=29&cmd_mod=2&host=xmlserver1.app.stage.redhat.com&btnSubmit=Commit
        ...
        try:
            response = urllib2.urlopen(self.nagios_instance, urllib.urlencode(opts))
        except:
    ...
    def _call_nagios_comment(self, host, comment, command):
        """
        build up the POST call for a host comment. Example of what nagios expects to get:
        cmd_typ=1&cmd_mod=2&host=app02.web.stage.ext.phx2.redhat.com&persistent=on&com_data=test+post&btnSubmit=Commit
        XXX: need to combine with other call methods
        """
        opts = {}
        opts['host'] = host 
    ...
    def _call_nagios_delete_all_comments(self, host, command):
        """
        build up the POST call for a host comment. Example of what nagios expects to get:
        cmd_typ=20&cmd_mod=2&host=app02.web.stage.ext.phx2.redhat.com&btnSubmit=Commit
        XXX: need to combine with other call methods
        """
        opts = {}
        opts['host'] = host
